...
class TradingSafeguards:
    ...
    def check_drawdown_limits(self) -> bool:
        ...
        if self._heat.max_drawdown > self._max_drawdown_threshold:
            logger.error(
                f"Max drawdown exceeded: {self._heat.max_drawdown*100:.2f}% > {self._max_drawdown_threshold*100}%"
            )
            self.activate_kill_switch(
                f"Drawdown limit exceeded: {self._heat.max_drawdown*100:.2f}%"
            )
            return False
        ...
        if self._heat.daily_loss < -self._daily_loss_threshold:
            ...
            self.activate_kill_switch(
                f"Daily loss limit exceeded: {self._heat.daily_loss*100:.2f}%"
            )
            return False
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "kill_switch": {"active": self._kill_switch_active, "reason": self._kill_switch_reason},
            "circuit_breakers": {
                name: {
                    "state": breaker.get_state(),
                    "failures": breaker.state.failure_count,
                    "last_failure": (
                        breaker.state.last_failure.isoformat()
                        if breaker.state.last_failure
                        else None
                    ),
                }
        ...
        while True:
            try:
                ...
                if self._heat.max_drawdown > self._max_drawdown_threshold * 0.8:
                    logger.warning(
                        f"Approaching drawdown limit: {self._heat.max_drawdown*100:.2f}%"
                    )

                if self._heat.daily_loss < -self._daily_loss_threshold * 0.8:
                    logger.warning(
                        f"Approaching daily loss limit: {self._heat.daily_loss*100:.2f}%"
                    )
...
async def handle_kill_switch_command(
    safeguards: TradingSafeguards, action: str, reason: str = ""
) -> str:
