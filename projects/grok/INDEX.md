# Grok project index

| Path | Role |
|---|---|
| [README.md](./README.md) | Hub entry |
| [LOOP.md](./LOOP.md) | Recurring steering loop |
| [TASKBOARD.md](./TASKBOARD.md) | Live next actions (generated) |
| [AUTOMATIONS.md](./AUTOMATIONS.md) | Automation inventory (generated) |
| [data/free_reign_policy.json](./data/free_reign_policy.json) | Default multi-rail policy |
| [data/dens_permanent.json](./data/dens_permanent.json) | Dens + dust no-rebuy |
| [data/axti_playbook.json](./data/axti_playbook.json) | AXTI scale-out rules |
| [data/genome_seed_lessons.json](./data/genome_seed_lessons.json) | Seed lessons |
| [data/windows_acceptance.json](./data/windows_acceptance.json) | Win DC probe state template |
| [data/taskboard.json](./data/taskboard.json) | Machine taskboard |
| [fixtures/](./fixtures/) | Research-worker manifests |
| [../../lib/grok/](../../lib/grok/) | Python package |
| [../../data/grok-web-exports/](../../data/grok-web-exports/) | Knowledge bridge store |
| [../../docs/strategy/WINDOWS-DATACENTER-MASTERPLAN-2026-08-06.md](../../docs/strategy/WINDOWS-DATACENTER-MASTERPLAN-2026-08-06.md) | Mission |

| [SYSTEM_BRIEF.md](./SYSTEM_BRIEF.md) | Unified alpha+policy+bridge brief |
| [PLANT_WIRE_POLICY.md](./PLANT_WIRE_POLICY.md) | Claude free-reign wire recipe |
| [NEXT.md](./NEXT.md) | What next |

| [data/blindspots.json](./data/blindspots.json) | Blindspot scoreboard |
| [data/playbooks.json](./data/playbooks.json) | Trading playbooks |
| [../../docs/strategy/HOLISTIC-BLINDSPOTS-AND-LEVERAGE-2026-08-06.md](../../docs/strategy/HOLISTIC-BLINDSPOTS-AND-LEVERAGE-2026-08-06.md) | Holistic strategy |

| [data/drive_bridge_folders.json](./data/drive_bridge_folders.json) | Google Drive Grok Bridge folder IDs |
| [../../docs/handoffs/GROK-DRIVE-BRIDGE-2026-08-06.md](../../docs/handoffs/GROK-DRIVE-BRIDGE-2026-08-06.md) | Drive densify bridge |
| [../../data/grok-drive-pack/](../../data/grok-drive-pack/) | Generated Drive pack (sanitized) |
