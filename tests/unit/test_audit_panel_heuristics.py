...
def test_oversize_diff_trips_only_when_lines_and_file_count_cross_threshold():
    pr = MergedPR(
        1,
        "feat: giant lane [skip ci]",
        additions=1901,
        deletions=200,
        changed_files=tuple(f"lib/x/{i}.py" for i in range(16)),
        commits=("feat: giant lane [skip ci]",),
    )
...
def test_oversize_diff_ignores_large_line_count_with_few_files():
    pr = MergedPR(
        1,
        "feat: generated docs [skip ci]",
        additions=2500,
        changed_files=("docs/a.md",),
        commits=("feat: generated docs [skip ci]",),
    )
...
def test_oversize_diff_ignores_many_files_with_small_delta():
    pr = MergedPR(
        1,
        "chore: touch fixtures [skip ci]",
        additions=20,
        deletions=20,
        changed_files=tuple(f"tests/{i}.py" for i in range(20)),
        commits=("chore: touch fixtures [skip ci]",),
    )
...
def test_weak_commit_message_accepts_specific_subject():
    pr = MergedPR(
        2, "feat: audit panel [skip ci]", commits=("feat: audit panel scoring [skip ci]",)
    )
...
def test_kill_switch_touch_without_owner_review_is_critical():
    ...
    assert [
        ...
        if finding.rule_id == "kill_switch_touched_without_review"
    ] == ["critical"]
...
def test_kill_switch_rule_ignores_unrelated_core_file():
    pr = MergedPR(
        3,
        "fix: heartbeat [skip ci]",
        changed_files=("lib/core/heartbeat.py",),
        commits=("fix: heartbeat [skip ci]",),
    )
...
def test_tests_missing_for_new_lib_module_trips():
    pr = MergedPR(
        ...
        changed_files=("lib/audit_panel/heuristics.py",),
...
def test_tests_missing_for_plugin_accepts_plugin_test():
    pr = MergedPR(
        ...
        changed_files=(
            "plugins/claw-sapphire/tools/internal/audit_panel.py",
            "plugins/claw-sapphire/tests/test_audit_panel.py",
        ),
...
def test_scope_creep_trips_when_narrow_title_touches_many_areas():
    pr = MergedPR(
        ...
        changed_files=(
            ...
            "docs/c.md",
            "infra/x.yaml",
            "plugins/claw-sapphire/tools/x.py",
        ),
...
def test_scope_creep_allows_small_surface():
    pr = MergedPR(
        5,
        "fix: dashboard card [skip ci]",
        changed_files=("services/dashboard/app.py", "tests/unit/test_dashboard.py"),
        commits=("fix: dashboard card [skip ci]",),
    )
...
def test_secret_signature_trips_on_key_like_diff_metadata():
    pr = MergedPR(
        ...
        diff_snippets=("OPENAI_API_KEY=sk-abcdefghijklmnopqrstuvwxyz",),
    )
...
def test_secret_signature_trips_on_private_key_marker():
    pr = MergedPR(
        ...
        "fix: config [skip ci]",
        commits=("fix: config [skip ci]",),
        diff_snippets=("-----BEGIN PRIVATE KEY-----",),
    )
...
def test_secret_signature_ignores_placeholder():
    pr = MergedPR(
        6,
        "docs: config example [skip ci]",
        commits=("docs: config example [skip ci]",),
        diff_snippets=("OPENAI_API_KEY=<set-in-env>",),
    )
...
def test_provenance_missing_trips_for_data_json():
    pr = MergedPR(
        7,
        "feat: artifact [skip ci]",
        commits=("feat: artifact [skip ci]",),
        changed_files=("data/a/report.json",),
    )
...
def test_provenance_missing_accepts_sibling_sidecar():
    pr = MergedPR(
        ...
        "feat: artifact [skip ci]",
        commits=("feat: artifact [skip ci]",),
        changed_files=("data/a/report.json", "data/a/report.json.envelope.json"),
    )
...
def test_provenance_missing_ignores_non_data_json():
    pr = MergedPR(
        7,
        "test: fixture [skip ci]",
        commits=("test: fixture [skip ci]",),
        changed_files=("tests/fixtures/report.json",),
    )
...
def test_failed_check_overridden_trips_on_failure():
    pr = MergedPR(
        8, "fix: bypass [skip ci]", commits=("fix: bypass [skip ci]",), status_conclusion="FAILURE"
    )
...
def test_failed_check_overridden_trips_on_timeout():
    pr = MergedPR(
        8,
        "fix: bypass [skip ci]",
        commits=("fix: bypass [skip ci]",),
        status_conclusion="TIMED_OUT",
    )
...
def test_failed_check_overridden_ignores_success():
    pr = MergedPR(
        8, "fix: normal [skip ci]", commits=("fix: normal [skip ci]",), status_conclusion="SUCCESS"
    )
