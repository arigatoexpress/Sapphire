...
def _model():
    return ImpactModel(
        ...
        horizons_hours=(1, 24),
        profiles=(
            ImpactProfile("fomc_decision", "rate_hike", "BTC", 24, -1.2, -1.0, 8, 2.0, (-3.0, 0.6), -0.5),
...
def test_macro_event_from_dict_parses_timestamp():
    event = MacroEvent.from_any({"title": "x", "category": "fomc_decision", "timestamp": "2024-01-01T00:00:00Z"})
...
def test_lookup_prefers_exact_profile():
    reaction = lookup({"category": "fomc_decision", "sub_category": "rate_hike"}, "BTC", 24, model=_model())
    assert reaction.matched_level == "exact"
    assert reaction.mean_return_pct == -1.2


def test_lookup_falls_back_to_category_profile():
    reaction = lookup({"category": "fomc_decision", "sub_category": "dovish_hold"}, "BTC", 24, model=_model())
    assert reaction.matched_level == "category"
    assert reaction.n == 30


def test_lookup_returns_wide_band_when_no_data():
    reaction = lookup({"category": "etf_approval", "sub_category": "spot_btc_etf_approval"}, "SOL", 24, model=_model())
...
def test_lookup_accepts_model_dict():
    reaction = lookup({"category": "fomc_decision", "sub_category": "rate_hike"}, "BTC", 24, model=_model().to_dict())
    assert reaction.matched_level == "exact"


def test_lookup_normalizes_asset_case():
    reaction = lookup({"category": "macro_shock", "sub_category": "bank_failure"}, "eth", 1, model=_model())
    assert reaction.asset == "ETH"


def test_lookup_confidence_medium_for_large_n():
    reaction = lookup({"category": "fomc_decision", "sub_category": "other"}, "BTC", 24, model=_model())
...
def test_lookup_unknown_horizon_returns_no_data():
    reaction = lookup({"category": "fomc_decision", "sub_category": "rate_hike"}, "BTC", 168, model=_model())
    assert reaction.n == 0
