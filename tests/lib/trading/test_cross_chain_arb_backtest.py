...
def test_decay_yields_lower_pnl_than_flat() -> None:
    ...
    flat = CrossChainArbBacktest(**common, decay_days=0).simulate_with_spread(
        "USDC", 500.0
    )
    decayed = CrossChainArbBacktest(**common, decay_days=7.0).simulate_with_spread(
        "USDC", 500.0
    )
...
def test_simulate_from_signal_picks_larger_side() -> None:
    ...
    bt = CrossChainArbBacktest(
        capital_usd=1_000_000.0, days=30, decay_days=0,
        bridge_cost_bps_per_round_trip=0, gas_cost_usd_per_action=0,
    )
...
def test_project_pnl_for_signal_convenience() -> None:
    ...
    via_class = CrossChainArbBacktest(
        capital_usd=10_000.0, days=30
    ).simulate_from_signal(sig)
    assert via_func.total_pnl_usd == pytest.approx(via_class.total_pnl_usd)
    assert via_func.cost_basis_pct_of_pnl == pytest.approx(
        via_class.cost_basis_pct_of_pnl
    )
...
def test_max_drawdown_present_when_costs_exceed_yield() -> None:
    ...
    bt = CrossChainArbBacktest(
        capital_usd=10_000.0, days=30, decay_days=7.0,
        bridge_cost_bps_per_round_trip=5.0, gas_cost_usd_per_action=2.50,
    )
    r = bt.simulate_with_spread("USDC", initial_spread_bps=232.0)
...
def test_sortino_inf_when_no_losing_periods() -> None:
    ...
    bt = CrossChainArbBacktest(
        capital_usd=10_000.0, days=30, decay_days=0,
        bridge_cost_bps_per_round_trip=0, gas_cost_usd_per_action=0,
    )
    r = bt.simulate_with_spread("USDC", initial_spread_bps=500.0)
