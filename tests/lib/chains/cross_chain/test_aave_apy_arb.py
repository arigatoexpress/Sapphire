...
async def test_low_spread_returns_none() -> None:
    ...
    scanner = CrossChainAaveScanner(
        ...
        arbitrum_protocols=_StubFacade(
            [_StubReserve("USDC", supply_apy=0.0510, borrow_apy=0.0710)]
        ),
        optimism_protocols=_StubFacade(
            [_StubReserve("USDC", supply_apy=0.0515, borrow_apy=0.0715)]
        ),
    )
...
async def test_100bps_spread_emits_opportunity_signal() -> None:
    ...
    scanner = CrossChainAaveScanner(
        ...
        arbitrum_protocols=_StubFacade(
            [_StubReserve("USDC", supply_apy=0.0500, borrow_apy=0.0600)]
        ),
        optimism_protocols=_StubFacade(
            [_StubReserve("USDC", supply_apy=0.0600, borrow_apy=0.0600)]
        ),
    )
    sig = await scanner.scan_asset("USDC")
...
async def test_500bps_spread_emits_extreme_signal() -> None:
    ...
    scanner = CrossChainAaveScanner(
        ...
        arbitrum_protocols=_StubFacade(
            [_StubReserve("WETH", supply_apy=0.0200, borrow_apy=0.0300)]
        ),
        optimism_protocols=_StubFacade(
            [_StubReserve("WETH", supply_apy=0.0200, borrow_apy=0.0800)]
        ),
    )
...
async def test_200bps_boundary_is_extreme() -> None:
    ...
    scanner = CrossChainAaveScanner(
        ...
        arbitrum_protocols=_StubFacade(
            [_StubReserve("USDT", supply_apy=0.0500, borrow_apy=0.0500)]
        ),
        optimism_protocols=_StubFacade(
            [_StubReserve("USDT", supply_apy=0.0700, borrow_apy=0.0500)]
        ),
    )
...
async def test_asset_on_two_chains_with_spread_still_emits() -> None:
    ...
    scanner = CrossChainAaveScanner(
        megaeth_protocols=_StubFacade([_StubReserve("USDC", supply_apy=0.0500, borrow_apy=0.0600)]),
        arbitrum_protocols=_StubFacade(
            [_StubReserve("USDC", supply_apy=0.0700, borrow_apy=0.0600)]
        ),
        optimism_protocols=_StubFacade(
            [_StubReserve("WETH", supply_apy=0.0200, borrow_apy=0.0300)]
        ),
