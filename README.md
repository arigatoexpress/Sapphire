...
**Model aliases:** `fast`/`quick` → nemotron-mini · `auto`/`balanced` → hermes3:8b · `deep` → qwen3:14b · `code` → gemma4 · `reason` → deepseek-r1:14b · `qwen-reason` → qwen3.5:9b · `qwen3.6` → qwen3.6:27b (Windows primary, Mac exact fallback, explicit only) · `cascade`/`moe` → nemotron-cascade-2 · `large` → qwen2.5:32b · `kimi` → kimi-cloud.
