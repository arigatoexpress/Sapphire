...
| ~~`com.sapphire.kronos-daily`~~    | ~~07:00 CT~~       | ~~`scripts/kronos_daily_predictions.py`~~   | ~~`data/intelligence/YYYY-MM-DD/predictions.json`~~                | **ARCHIVED 2026-05-12** — plist moved to `_Archive_2026-05-12/launchagent-stale-plists/`; not loaded by launchd. Run manually if needed: `python3 scripts/kronos_daily_predictions.py`; GPU must be up |
...
| `com.sapphire.trading-shadow-controller` | every 30 min | `scripts/ops/trading_shadow_controller.py --output --history` | `data/trading/shadow-controller-latest.json`; append-only `data/trading/shadow-controller-history.jsonl` | Manual paper-only run: `python3 scripts/ops/trading_shadow_controller.py --output --history`; no Robinhood credentials or `--execute` flag |
| `com.sapphire.continuous-intelligence-daily` | 06:45 local | `python3 -m lib.autonomy.continuous_intelligence_artifacts daily-packet --write` | `data/.autonomy/continuous_intelligence/daily_autonomy_packet_latest.json`; append-only task/lease/packet JSONL | Manual artifact-only run: `python3 -m lib.autonomy.continuous_intelligence_artifacts daily-packet --write --pretty`; no worker dispatch, Telegram send, or trading action |
