...
- Every address in this document was either (a) read from one of those three sources verbatim, or (b) live-checked with `eth_getCode` to confirm bytecode is present at the address. A refresh against [`api.llama.fi/v2/chains`](https://api.llama.fi/v2/chains) on 2026-07-30 reported `MegaETH` total TVL at $46.85M, 80.9% below its 2026-05-12 peak. Treat the launch-day values in the historical rows below as snapshots, not current capital-allocation evidence.
...
| 10 | **MegaUSD (USDm) — bridged LayerZero V2 OFT** | stable (bridged OFT) | n/a (token = $360.75M circulating at TGE day, 18 decimals) | Token (ERC1967 proxy → `xUSDOFTUpgradeable` impl) `0xFAfDdbb3FC7688494971a79cc65DCa3EF82079E7` · TWAP-60 oracle `MegaEthPriceFeedUsdmTwap60WithoutRoundsV2` `0x663B50C9DA9Bd586f855aF13e91EF2f0954c9761` · composite ETH-pair oracle `MegaEthPriceFeedUsdm…Eth` `0xD011778057AA740BB3703Ad4d78b3c79a1aED1cb` · Aave V3 `Oracle.getAssetPrice(USDM)` (third independent feed) | Blockscout-verified, [LayerZero V2 OFT spec](https://docs.layerzero.network/v2/concepts/applications/oft-standard) | 1 | **Live USDm is bridged in via LayerZero V2 OFT — not minted on-chain.** Wave B.2 (PR #537) shipped the read wrapper + 3-source peg monitor. Collateral lives off-chain at the source-chain issuer; on-chain reads here observe **symptoms** (peg deviation, mint/burn events, blacklist activity) not **causes** (collateral solvency). See [§ Notes on bridged tokens](#notes-on-bridged-tokens-layerzero-oft-semantics) below. |
| 10a | ⚠️ **`Minter_v2 + StabilityPool_v2 + ReservePool_v1 + Genesis_v1` stack — UNINITIALIZED, DO NOT INTEGRATE** | (was: stable CDP-style) | n/a | `Minter_v2` `0x2A9bd9829B690Ee36B79f735412E0959F7813534` · `StabilityPool_v2` `0xaa47742C019357c7DD85917A4126676265D12EBE` · `ReservePool_v1` `0xa3287C1A9A777426bC3022d2a94AfDA9561bda3a` · `Genesis_v1` `0x6774745AfC1574Cda36D7cc2D53Df0028480790e` | Blockscout-verified | — | ⚠️ **DO NOT INTEGRATE.** Earlier drafts of this map listed these four contracts as the USDm CDP stack. Live probes on 2026-04-30 (PR #537) proved they target a *different, uninitialized* pegged-token: `Minter_v2.PEGGED_TOKEN()` returns `0xbed2c…`, NOT the live USDm at `0xFAfDdbb…`; `collateralRatio()`, `leverageRatio()`, `leveragedTokenPrice()`, `harvestable()` all REVERT (`priceOracle()` is the zero address); `peggedTokenBalance()`/`collateralTokenBalance()`/`leveragedTokenBalance()` all return `0`; `StabilityPool_v2.totalAssetSupply()` returns `0`. Listed only so future readers don't re-discover the wrong addresses and assume they describe live USDm. |
...
### 2.4 Stablecoins — USDm (LayerZero OFT, bridged), USDT0 (Tether), Ethena USDe

- **Read-only (Wave B.2 — already shipped, PR #537)**: ERC-20 `totalSupply` / `circulating_supply` (totalSupply minus burn-address balances) / `is_blacklisted(account)` / `is_minter(account)` from `lib/chains/megaeth/contracts/usdm.py`; `peg_quote()` composing **three independent USD-price sources** — Kumbaya USDm/USDT0 0.01% pool (venue, fastest to break on depeg), `MegaEthPriceFeedUsdmTwap60` on-chain TWAP-60 (slow but manipulation-resistant), `AaveOracle.getAssetPrice(USDm)` (the price Aave V3 actually liquidates against) — plus a fourth composite ETH-pair feed at `0xD011778…`. `peg_monitor.py` classifies divergence into severity bands (`HEALTHY`, `WARNING_50BP`, `BREAK_100BP`, `CRISIS_500BP`, `CIRCUIT_BREAKER`).
- **NOT readable on-chain** (collateral lives off-chain at the LayerZero source — see § below): `collateral_ratio`, `mint_paused`, `redeem_paused`. The wrapper returns `None` for each rather than faking a reassuring value.
- **Sapphire surfaces**: `lib/chain/intelligence.py` consumes `MegaETHProtocols.stable_health()` for the "stable health" panel. Peg deviation crossing the `BREAK_100BP` threshold → `chain.regime.shift` event-bus emit (analogous to Lido stETH/eETH monitoring on L1).
- **Execution**: USDm mint/redeem are NOT executable from MegaETH — bridge in/out via LayerZero V2. Wave C will gate any "increase USDm exposure" decision on `peg_status == HEALTHY` AND `is_safe_for_leverage()` returning true.

#### Notes on bridged tokens (LayerZero OFT semantics)

Live USDm at `0xFAfDdbb…` is an **OmniFungibleToken (OFT)** behind an `ERC1967Proxy`, not a CDP-mint or PSM-mint. The collateral that backs USDm sits at the source chain (the OFT root) and is custodied by the issuer (M^0 / Mountain or whoever owns the upstream root). Three implications for any Sapphire surface that reads or trades against USDm on MegaETH:

1. **No on-chain collateral observability.** There is no `collateralRatio()`, no `paused()`, no on-chain reserve attestation accessible from chain 4326. Anything that asserts "USDm is N% over-collateralized" using only MegaETH RPC reads is fabricating that number. Wave B.2's wrapper deliberately returns `None` for those views.
2. **Symptoms, not causes.** The peg-monitor reads price-divergence symptoms (Kumbaya pool moving away from $1, AaveOracle drifting from TWAP-60, Aave-vs-venue spread widening). A real depeg shows up there *before* the issuer publishes a collateral incident — but the converse is also true: a transient liquidity event on the Kumbaya pool can register as `WARNING_50BP` even when the issuer's collateral is fine. The severity bands are calibrated to widen the gap between "noise" and "real distress" but they're a leading-indicator surface, not a definitive solvency check.
3. **Mint/burn events are bridge flow.** On an OFT, the `Burned` event covers both true redemptions and cross-chain bridge-outs (the OFT burns on the source chain when sending). `recent_mints()` / `recent_redeems()` should be read as **net bridge-flow direction**, not as native mint/redeem rate. A spike in `Burned` paired with a falling Kumbaya price is a much stronger distress signal than either in isolation.

The historical confusion: an earlier version of this document listed `Minter_v2` + `StabilityPool_v2` + `ReservePool_v1` + `Genesis_v1` as the "USDm stack." Those four contracts are deployed and verified, but live probes (PR #537) confirm they target a *different, uninitialized* pegged-token (their `PEGGED_TOKEN()` returns `0xbed2c…`, not `0xFAfDdbb…`). They are listed in row 10a above with a hard "DO NOT INTEGRATE" warning so the wrong addresses are not re-adopted by future work.
...
| ~~B-3~~ → **B.2 (shipped, PR #537)** | `feat(megaeth): USDM stable health + 3-source peg monitor` | `lib/chains/megaeth/contracts/usdm.py`, `lib/chains/megaeth/peg_monitor.py`, `abis/usdm/{token,oracle_usdm_twap60,oracle_usdm_eth}.json` + meta. **Note:** original plan listed `Minter_v2`/`StabilityPool_v2`/`ReservePool_v1` ABIs; those targets turned out to be uninitialized (see § 2.4 + row 10a). Live USDm is bridged via LayerZero OFT, so the wrapper reads OFT events + 3 independent oracle feeds instead. Bridge-predeploy wrapper deferred to a separate PR. | shipped | low |
