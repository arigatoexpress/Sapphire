...
<Example structure:
- 04:06:00 UTC — quote read; spread 4 bps; bid $76,787; ask $76,791
- 04:06:03 UTC — order draft created (limit buy 0.00006511 BTC at $76,787)
- 04:06:08 UTC — confirmation token issued, operator entered token in active terminal
- 04:06:10 UTC — order submitted via v2 crypto endpoint
- 04:06:13 UTC — fill confirmation: 0.00006511 BTC at $76,774.81, fee $0.04
- 04:06:13 UTC — observed: fill price $12.19 BELOW limit price; expected near-limit fill
>
...
<Example: per `docs/products/live-trading-ramp-memo.md` Phase 1, a $5 limit-buy on BTC should fill at or near limit price. Sub-limit fills are favorable but the magnitude here ($12.19) is large enough relative to the limit ($76,787) that it warrants explanation.>
...
<Example: fill came in $12.19 below limit. This is FAVORABLE. But: the magnitude (~16 bps) is consistent with crypto-market liquidity gaps in the after-hours window (04:06 UTC), not with the spread observed at quote-read time (4 bps). Implication: the spread we read is NOT the spread we got.>

## 4. The fix

<What you did or are going to do, immediately, to prevent this incident from recurring before the next live order.>

<Example: tighten the just-in-time quote re-read to within 1 second of order submit; flag any quote-to-fill spread delta > 10 bps for post-trade review. Add an "after-hours liquidity warning" to the dry-run output if order is between 02:00–10:00 UTC.>
...
- Common edits:
  ...
  - For exchange-side weirdness (Robinhood returns 5xx, partial fill, unexpected order rejection), Section 2 should cite the v2 crypto endpoint contract directly.
...
- DO file even for FAVORABLE gaps. A $12.19 favorable gap is a 16 bps win this time and a 16 bps loss next time. Pattern data is the point.
