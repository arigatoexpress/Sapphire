...
Thanks — appreciate the reply. Let's do 30 minutes.
...
To prep your team in advance:

- Acquirer microsite (one-pager + links): {{microsite_url}}
- Diligence packet (10 docs — architecture, security, data, ops, financials, roadmap, incidents, team/process): {{diligence_pr_url}}
- Pitch deck (if useful, but the diligence packet is denser): {{deck_url}}
...
- Why this works: agrees to the call without negotiating the ask, sets the agenda explicitly (corp-dev calls without a structure get hijacked by the rep's template questions), and arms them with materials to share internally before the meeting.
- Common edits: drop the "five minutes on context" if the rep already explained what they care about in their reply — re-asking signals you didn't read. Replace with deeper dive on whatever they named.
- Tone calibration: agenda-forward, not deferential. Corp-dev calls go best when the founder runs the meeting; this template signals you intend to.
- Do NOT promise specific commercial terms in this email. Those are for the call, after they've seen the artifacts.
