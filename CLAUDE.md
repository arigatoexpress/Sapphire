...
```bash
# Test
pytest tests/unit/ --tb=short -q           # 6,999 collected by test_inventory.py (2026-07-28, local)
pytest plugins/claw-sapphire/tests/ -q     # 637 collected by test_inventory.py (2026-07-28)
...
- OKX is behind Cloudflare: the default `Python-urllib` User-Agent gets HTTP 403 "error 1010 (browser signature)" on **every** endpoint, including unauthenticated market data. `lib/portfolio/okx.py` sets a browser UA — don't remove it. OKX also returns HTTP 200 with a non-zero application-level `code`, so success must be checked on the body, not the status.
- Robinhood lists crypto holdings its own `/marketdata/best_bid_ask/` endpoint refuses to quote (currently JTO, VVV, MEGA, CC, GRAM), which arrive with `price=None` and silently vanish from portfolio totals. `PortfolioAggregator` backfills these from OKX public tickers and tags the row `price_source: "okx_public"`; anything still unpriced stays `None` (never $0) and is counted in `sources[].unpriced_count`.
- `/api/portfolio` `cash` includes stablecoin holdings *and* those coins are also rows in `holdings` — consumers must read `total_value` rather than recomputing `cash + Σ market_value`, which double-counts stables.
