...
- **Tier 3**: Mac Ollama (hermes3:8b, nemotron-mini, qwen3.6:27b exact fallback)
- **Tier 4**: Kimi Cloud (non-sensitive queries only, sensitivity-gated)

Model tier aliases: `fast`→nemotron-mini, `balanced`→hermes3:8b, `deep`→qwen3:14b, `code`→gemma4:latest, `reason`→deepseek-r1:14b, `qwen-reason`→qwen3.5:9b, `qwen3.6`→qwen3.6:27b on Mac fallback
