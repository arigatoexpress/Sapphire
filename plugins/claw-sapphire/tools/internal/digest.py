        ...
        return {
            ...
            "quant": {
                sym: {
                    ...
                    "trend_direction": q.trend_direction,
                    "nearest_support": q.nearest_support,
                    "nearest_resistance": q.nearest_resistance,
...
def generate_digest() -> str:
    ...
    if "technical" in market:
        ...
        for sym, data in market["technical"].items():
            icon = (
                "🟢"
                if "bullish" in data["net_signal"]
                else "🔴"
                if "bearish" in data["net_signal"]
                else "⚪"
            )
            lines.append(
                f"  {icon} {sym} ${data['price']:,.2f} | {data['net_signal'].upper()} | RSI {data['rsi']:.0f} | MACD {data['macd_cross']}"
            )
        ...
        if "quant" in market:
            ...
            for sym, q in market["quant"].items():
                lines.append(
                    f"  {sym}: S=${q['nearest_support']:,.2f} R=${q['nearest_resistance']:,.2f} R:R={q['risk_reward']:.1f}"
                )
