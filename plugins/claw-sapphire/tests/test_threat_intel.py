...
_spec = importlib.util.spec_from_file_location(
    "threat_intel", TOOLS / "internal" / "threat_intel.py"
)
...
def test_action_scan_summarizes_critical_records(monkeypatch):
    ...
    fake_sources = SimpleNamespace(
        collect_latest_records=lambda days=3, per_source=5: list(records)
    )
    fake_scoring = SimpleNamespace(
        record_priority=lambda record: (record.score or 0) + (50 if record.exploited else 0)
    )
