...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --timeout "${TIMEOUT}" \
  --port "${PORT}" \
  --set-env-vars "SAPPHIRE_FULL_AUTONOMY_ENABLED=${FULL_AUTONOMY_ENABLED}" \
  --set-env-vars "SAPPHIRE_AUTONOMY_ALLOW_CODE_CHANGES=${AUTONOMY_ALLOW_CODE_CHANGES}" \
  --set-env-vars "SAPPHIRE_AUTONOMY_ALLOW_GCLOUD_CHANGES=${AUTONOMY_ALLOW_GCLOUD_CHANGES}" \
  --set-env-vars "SAPPHIRE_AUTONOMY_DRY_RUN=${AUTONOMY_DRY_RUN}" \
  --set-env-vars "SAPPHIRE_AUTONOMY_REQUIRE_OWNER_APPROVAL=${AUTONOMY_REQUIRE_OWNER_APPROVAL}" \
  --set-env-vars "SAPPHIRE_AUTONOMY_LOOP_SECONDS=${AUTONOMY_LOOP_SECONDS}" \
  --set-env-vars "SAPPHIRE_AUTONOMY_MIN_DISPATCH_INTERVAL_SECONDS=${AUTONOMY_MIN_DISPATCH_INTERVAL_SECONDS}" \
  --set-env-vars "GEMINI_FLASH_MIN_INTERVAL_SECONDS=${GEMINI_FLASH_MIN_INTERVAL_SECONDS}" \
  --set-env-vars "TRADINGVIEW_EXECUTION_ENABLED=${TRADINGVIEW_EXECUTION_ENABLED}" \
  --set-env-vars "TRADINGVIEW_DEFAULT_QUANTITY=${TRADINGVIEW_DEFAULT_QUANTITY}" \
  --set-env-vars "SAPPHIRE_ALLOWED_REPOS=${SAPPHIRE_ALLOWED_REPOS}" \
  --set-env-vars "SAPPHIRE_ALLOWED_GCP_PROJECTS=${SAPPHIRE_ALLOWED_GCP_PROJECTS}" \
  --set-env-vars "SAPPHIRE_BLOCKED_SCOPE_TERMS=${SAPPHIRE_BLOCKED_SCOPE_TERMS}"
