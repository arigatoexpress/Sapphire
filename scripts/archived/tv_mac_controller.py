        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    value_elem = await item.query_selector(
                        '[class*="valueValue"], [class*="value"]'
                    )
        ...
        while self.connected:
            try:
                ...
                if int(datetime.now().timestamp()) % 30 == 0:
                    logger.info(
                        f"📊 {self.current_symbol or 'Unknown'} @ ${self.current_price:,.2f} | Indicators: {len(self.indicator_values)}"
                    )
