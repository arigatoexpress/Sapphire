...
class RiskManagementAgent:
    ...
    async def evaluate(self, signal: TradeSignal, portfolio: dict) -> tuple[bool, dict]:
        ...
        if current_heat + trade_heat > self.max_portfolio_heat:
            rejection_reasons.append(f"Portfolio heat would exceed {self.max_portfolio_heat*100}%")
            return False, {"rejection_reasons": rejection_reasons}
...
class StrategyAgent:
    ...
    async def evaluate(self, signal: TradeSignal) -> tuple[bool, dict]:
        ...
        strategy_stats = self.strategy_performance.get(signal.strategy, {
            "total_trades": 0,
            "win_rate": 0.5,
            "profit_factor": 1.0
        })
        ...
        if strategy_stats["total_trades"] > 10:
            if strategy_stats["win_rate"] < 0.4:
                rejection_reasons.append(f"Strategy {signal.strategy} win rate {strategy_stats['win_rate']:.1%} too low")
                return False, {"rejection_reasons": rejection_reasons}
            elif strategy_stats["win_rate"] > 0.6:
                approval_reasons.append(f"Strategy {signal.strategy} has strong track record: {strategy_stats['win_rate']:.1%} win rate")
        ...
        if signal.action in ["entry_long", "exit_short"] and self.market_regime == "bearish":
            approval_reasons.append("Counter-trend long in bearish regime - reduced size recommended")
    ...
    async def evaluate(self, signal: TradeSignal) -> tuple[bool, dict]:
        ...
        if signal.action in ["entry_long", "exit_short"]:
            if sentiment["sentiment_score"] < -0.3:
                rejection_reasons.append(f"Bearish sentiment ({sentiment['sentiment_score']:.2f}) contradicts long signal")
                return False, {"rejection_reasons": rejection_reasons, "sentiment": sentiment}
            elif sentiment["sentiment_score"] > 0.5:
                approval_reasons.append(f"Bullish sentiment ({sentiment['sentiment_score']:.2f}) confirms long signal")
                
        elif signal.action in ["entry_short", "exit_long"]:
            if sentiment["sentiment_score"] > 0.3:
                rejection_reasons.append(f"Bullish sentiment ({sentiment['sentiment_score']:.2f}) contradicts short signal")
                return False, {"rejection_reasons": rejection_reasons, "sentiment": sentiment}
        ...
        if self.fear_greed_index < 20 and signal.action in ["entry_short"]:
            ...
            return False, {"rejection_reasons": rejection_reasons, "fear_greed": self.fear_greed_index}
...
class AIOrchestrator:
    ...
    async def process_signal(self, signal: TradeSignal) -> TradeDecision:
        ...
        decision = TradeDecision(
            signal=signal,
            status=TradeStatus.AI_VALIDATING
        )
        ...
        if not strategy_ok:
            ...
            decision.reasoning = f"Strategy rejection: {strategy_result.get('rejection_reasons', [])}"
        ...
        if not sentiment_ok:
            ...
            decision.reasoning = f"Sentiment rejection: {sentiment_result.get('rejection_reasons', [])}"
