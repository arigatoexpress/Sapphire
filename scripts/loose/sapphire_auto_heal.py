...
class AutoHealSystem:
    ...
    def __init__(self):
        ...
        self.pi_services = {
            ...
            "pi1_aster": {"host": "192.168.1.23", "service": "aster-trading", "auto_heal": True},
            "pi2_lighter": {
                "host": "192.168.1.173",
                "service": "lighter-trading",
                "auto_heal": True,
    ...
    async def check_service(self, name: str, config: dict) -> bool:
        ...
        try:
            ...
            async with (
                ...
                session.get(config["check_url"], timeout=aiohttp.ClientTimeout(total=3)) as resp,
            ):
    ...
    async def heal_mac_service(self, name: str, config: dict):
        ...
        # Start new process
        subprocess.Popen(
            config["start_cmd"], shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        ...
        self.heal_history.append(
            {"timestamp": datetime.now().isoformat(), "service": name, "action": "restarted"}
        )
