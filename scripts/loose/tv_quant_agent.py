...
class TVQuantAgent:
    ...
    async def add_indicator(self, indicator_name: str, params: dict = None):
        ...
        self.execute_js(
            "document.querySelector('[data-name=\"indicator-search-button\"]')?.click()"
        )
    ...
    async def change_symbol(self, symbol: str):
        ...
        self.execute_js(
            "document.querySelector('[data-name=\"symbol-search-input\"] input')?.select()"
        )
    ...
    async def run_quantitative_benchmark(self, symbols: list[str], strategies: list[str]) -> dict:
        ...
        benchmark_results["best_performers"] = [
            {"name": name, "profit": data.get("netProfit")} for name, data in sorted_strategies[:3]
        ]
...
async def start_quant_api(agent: TVQuantAgent, port: int = 8084):
    ...
    async def strategies(request):
        return web.json_response(
            {"tested": list(agent.strategy_results.keys()), "count": len(agent.strategy_results)}
        )
