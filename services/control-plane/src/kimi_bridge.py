...
def _overview(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    from .project_board import get_overview
    return {"status": "ok", "overview": get_overview()}


def _list_tasks(payload: dict[str, Any]) -> dict[str, Any]:
    from .control_plane import list_pending_tasks
    limit = int(payload.get("limit", 20))
    tasks = list_pending_tasks(limit=limit)
    return {"status": "ok", "tasks": tasks, "count": len(tasks)}


def _create_task(payload: dict[str, Any]) -> dict[str, Any]:
    from .control_plane import create_task
    title = str(payload.get("title", "")).strip()
    if not title:
        return {"status": "error", "error": "title required"}
    task = create_task(
        title=title,
        project=str(payload.get("project", "sapphire")),
        priority=str(payload.get("priority", "medium")),
        description=str(payload.get("description", "")),
        capability=str(payload.get("capability", "python")),
...
def _update_task(payload: dict[str, Any]) -> dict[str, Any]:
    from .control_plane import update_task_status
    task_id = str(payload.get("task_id", "")).strip()
    status = str(payload.get("status", "")).strip()
    if not task_id or not status:
        return {"status": "error", "error": "task_id and status required"}
    updated = update_task_status(task_id=task_id, status=status, result=payload.get("result"))
...
def _list_agents(payload: dict[str, Any]) -> dict[str, Any]:
    from .control_plane import list_registered_agents
...
def _sync_board(payload: dict[str, Any]) -> dict[str, Any]:
    from .project_board import get_overview
    return {"status": "ok", "board": get_overview()}
