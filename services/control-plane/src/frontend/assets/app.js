...
function renderSystemAnalysis(opsData, logbookData) {
  ...
  const executorRows = [
    ...
    `tools: gh=${Boolean(toolSummary.gh)} gcloud=${Boolean(toolSummary.gcloud)} docker=${Boolean(toolSummary.docker)} node=${Boolean(toolSummary.node)}`,
  ];
...
async function refreshOverview() {
  try {
    ...
    if (stateEls.signalList) {
      stateEls.signalList.innerHTML = '<p class="empty-state">Live overview unavailable. Check Cloud Run logs.</p>';
    }
