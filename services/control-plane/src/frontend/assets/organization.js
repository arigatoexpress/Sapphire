...
function renderExecutorTrust(opsInventory) {
  ...
  if (els.executorToolGrid) {
    const chips = [
      ...
      { label: "gcloud", ok: Boolean(tools.gcloud) },
...
function buildOrgArchitectureMermaid(payload) {
  ...
  const serviceName = String(payload?.shell_app?.service_name || "agentic-pm-hub");
  const rails = Array.isArray(payload?.trading_operations?.rails) ? payload.trading_operations.rails : [];
  ...
  const asterAgent = String(asterRail.preferred_agent_id || "claude-main");
  ...
  return `flowchart TB\n  TG[\"Telegram Control Plane\"] --> HUB[\"GCP Cloud Run\\\\n${serviceName}\"]\n  HUB --> SHELL[\"${shellName}\"]\n  HUB --> Q[\"Lease Queue + Router\"]\n  Q --> A1[\"${asterNode}\\\\n${asterAgent}\"]\n  Q --> A2[\"${lighterNode}\\\\n${lighterAgent}\"]\n  A1 --> V1[\"Aster Venue\"]\n  A2 --> V2[\"Lighter Venue\"]\n  HUB --> PRJ[\"Projects: AI PM | THO | Blanga | Sapphire\"]\n  HUB --> OBS[\"Architecture + Organization Dashboards\"]`;
}
...
async function loadOrganization(refresh = false) {
  ...
  setText(els.modelSubtitle, String(trading.description || "Cloud control + edge execution"));
  ...
  renderList(els.modelList, [
    ...
    `Shell app: ${String(payload.shell_app?.name || "Sapphire Command Deck")}`,
    `Runtime: ${String(payload.shell_app?.runtime_mode || "unknown")} | Service: ${String(payload.shell_app?.service_name || "local")}`,
