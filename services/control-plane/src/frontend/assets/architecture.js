...
function buildGcpPiMermaid(stats) {
  ...
  return `flowchart TB
  subgraph GCP["Google Cloud (sapphire-479610)"]
    RUN["Cloud Run: agentic-pm-hub"]
...
function buildAgentWorkflowMermaid(payload) {
  ...
  const serviceName = String(payload?.service_name || "agentic-pm-hub");
  const revision = String(payload?.service_revision || "unknown");
  const runtimeMode = String(payload?.runtime_mode || "cloud_run");
  ...
  subgraph CLOUD["Google Cloud (${runtimeMode})"]
    HUB["${serviceName}\\\\nrev:${revision}"]
    ROUTER["Policy + Membership Router"]
    QUEUE["Control Queue"]
    SECRETS["Secret Manager + Scheduler"]
  end
...
function buildSecurityBoundaryMermaid(payload, opsInventory) {
  ...
  KIMI --> HUB
  KIMI --- TOOLS["tools: gh=${Boolean(tools.gh)} | gcloud=${Boolean(tools.gcloud)} | docker=${Boolean(tools.docker)} | node=${Boolean(tools.node)}"]
...
function renderExecutorReadiness(opsInventory) {
  ...
  if (els.executorToolGrid) {
    const chips = [
      ...
      { label: "gcloud", ok: Boolean(toolSummary.gcloud) },
