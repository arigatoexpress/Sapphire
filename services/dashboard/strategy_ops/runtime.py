...
def build_strategy_ops_assessment(*, scorecard: dict, reject_tax: dict) -> dict:
    ...
    reject_tax_pct = float(
        reject_tax.get("reject_tax_pct", totals.get("reject_tax_pct", 0.0)) or 0.0
    )
    ...
    max_reject = max(
        0.0, min(100.0, float(_os.getenv("STRATEGY_OPS_GONOGO_MAX_REJECT_TAX_PCT", "70") or 70.0))
    )
    max_hard_fail = max(
        0.0, min(100.0, float(_os.getenv("STRATEGY_OPS_GONOGO_MAX_HARD_FAIL_PCT", "10") or 10.0))
    )
    ...
    promote_lanes = [
        row for row in ranked if str(row.get("recommendation", "")).lower() == "promote"
    ]
...
def resolve_strategy_ops_decision(
    *,
    ...
    assessment_reasons = (
        assessment.get("reasons", []) if isinstance(assessment.get("reasons"), list) else []
    )
    ...
    return {
        ...
        "source": source,
        "diverged": bool(
            assessment_label != label or assessment_go != go or operator_label != label
        ),
...
def build_go_no_go_brief_payload(*, ops_payload: dict, window_days: int, timestamp: str) -> dict:
    decision = (
        ops_payload.get("decision", {}) if isinstance(ops_payload.get("decision"), dict) else {}
    )
    operator_brief = (
        ops_payload.get("operator_brief", {})
        if isinstance(ops_payload.get("operator_brief"), dict)
        else {}
    )
    assessment = (
        ops_payload.get("assessment", {}) if isinstance(ops_payload.get("assessment"), dict) else {}
    )
    data_quality = (
        ops_payload.get("data_quality", {})
        if isinstance(ops_payload.get("data_quality"), dict)
        else {}
    )
    reject_tax = (
        ops_payload.get("reject_tax", {}) if isinstance(ops_payload.get("reject_tax"), dict) else {}
    )
    attribution = (
        ops_payload.get("attribution", {})
        if isinstance(ops_payload.get("attribution"), dict)
        else {}
    )
    scorecard = (
        ops_payload.get("scorecard", {}) if isinstance(ops_payload.get("scorecard"), dict) else {}
    )
    ...
    summary = {
        ...
        "expected_value_error_pct": round(
            _as_float(attribution.get("expected_value_error_pct"), 0.0), 2
        ),
    ...
    return {
        ...
        "top_lanes": (
            scorecard.get("ranked", []) if isinstance(scorecard.get("ranked"), list) else []
        )[:3],
        "sources": {
