...
def build_strategy_scorecard(
    *,
    ...
    for lane in agg.values():
        ...
        avg_conf = (
            _to_float(lane.get("signal_conf_sum"), 0.0)
            / max(1, int(lane.get("signal_conf_count", 0) or 0))
        ...
        avg_latency_ms = (
            _to_float(lane.get("latency_ms_sum"), 0.0)
            / max(1, int(lane.get("latency_ms_count", 0) or 0))
    ...
    totals = {
        ...
        "filled_success_count": sum(
            int(item.get("filled_success_count", 0) or 0) for item in ranked
        ),
    ...
    totals["expected_value_error_pct"] = round(
        (
            sum(
                _to_float(item.get("expected_value_error_pct"), 0.0)
                * max(1, int(item.get("sample_size", 0) or 0))
                for item in ranked
            )
            / max(
                1,
...
def build_signal_outcome_attribution(
    *,
    ...
    for row in trade_rows:
        ...
        metadata = row.get("metadata", {}) if isinstance(row.get("metadata"), dict) else {}
        signal_id = str(
            row.get("signal_id") or metadata.get("signal_id") or row.get("execution_id") or ""
        ).strip()
        if not signal_id:
            continue
    ...
    for idx, row in enumerate(verification_rows):
        metadata = row.get("metadata", {}) if isinstance(row.get("metadata"), dict) else {}
        signal_meta = (
            row.get("signal_metadata", {}) if isinstance(row.get("signal_metadata"), dict) else {}
        )
        strategy, timeframe = _strategy_lane_info(row)
        outcome = str(row.get("outcome", "")).strip().lower() or "unknown"
        reason_text = str(
            row.get("policy_reason") or row.get("noop_reason") or row.get("error_message") or ""
        ).strip()
        reason = _reason_code(outcome, reason_text)
        ts = _coerce_datetime(row.get("recorded_at") or row.get("timestamp")) or now_utc
        signal_id = str(
            row.get("signal_id") or signal_meta.get("signal_id") or metadata.get("signal_id") or ""
        ).strip()
        if not signal_id:
        ...
        side = (
            str(row.get("side") or signal_meta.get("side") or metadata.get("side") or "")
            .strip()
            .lower()
            or "unknown"
        )
        confidence = _safe_float(
            row.get("confidence") or signal_meta.get("confidence") or metadata.get("confidence"),
            0.0,
        )
        latency_ms = _safe_float(
            row.get("latency_ms") or row.get("processing_latency_ms") or metadata.get("latency_ms"),
            0.0,
        )
    ...
    for group in [*windows.values(), *by_lane.values()]:
        ...
        group["fill_rate_pct"] = round(
            (float(group.get("filled_success_count", 0)) / denom) * 100.0, 2
        )
        group["reject_tax_pct"] = round(
            (float(group.get("reject_skip_count", 0)) / denom) * 100.0, 2
        )
        group["hard_fail_pct"] = round(
            (float(group.get("hard_failed_count", 0)) / denom) * 100.0, 2
        )
        group["net_pnl_after_fees_usd"] = round(
            float(group.get("net_pnl_after_fees_usd", 0.0) or 0.0), 6
        )
        group["estimated_fees_usd"] = round(float(group.get("estimated_fees_usd", 0.0) or 0.0), 6)
        group["estimated_slippage_usd"] = round(
            float(group.get("estimated_slippage_usd", 0.0) or 0.0), 6
        )
...
def build_data_quality_snapshot(
    *,
    ...
    tracked = [
        str(sym).upper() for sym in (market.get("tracked_symbols") or []) if str(sym).strip()
    ]
    tracked_count = max(1, len(tracked))
    missing_quotes = sum(
        1 for sym in tracked if _safe_float((market.get(sym) or {}).get("price"), 0.0) <= 0
    )
    ...
    reason_rows = (
        attribution.get("by_reason", []) if isinstance(attribution.get("by_reason"), list) else []
    )
    unknown_share = 0.0
...
def build_operator_decision_brief(
    *,
    ...
    fill_rate = _safe_float(
        north_star.get("fill_rate_pct"), _safe_float(totals.get("fill_rate_pct"), 0.0)
    )
    reject_pct = _safe_float(
        north_star.get("reject_tax_pct"), _safe_float(reject_tax.get("reject_tax_pct"), 0.0)
    )
    ...
    drawdown_pct = _safe_float(
        north_star.get("max_drawdown_pct"), _safe_float(totals.get("max_drawdown_pct"), 0.0)
    )
    ...
    promote_lane = next(
        (row for row in ranked if str(row.get("recommendation", "")).lower() == "promote"), None
    )
    if isinstance(promote_lane, dict):
        lane_name = (
            f"{promote_lane.get('strategy', 'unknown')}@{promote_lane.get('timeframe', 'unknown')}"
        )
        top_actions.append(
            {
                "priority": "P1",
    ...
    weak_lane = next(
        (row for row in lanes if _safe_float(row.get("reject_tax_pct"), 0.0) >= 70.0), None
    )
    if isinstance(weak_lane, dict):
        lane_name = (
            f"{weak_lane.get('strategy', 'unknown')}@{weak_lane.get('timeframe', 'unknown')}"
        )
        top_actions.append(
            {
                "priority": "P0",
    ...
    confidence_score = _clamp(
        (50.0 + (fill_rate * 0.35) - (reject_pct * 0.30) - (drawdown_pct * 0.90))
    ...
    return {
        ...
        "thresholds": {
            ...
            "max_reject_tax_pct": float(
                (assessment.get("thresholds") or {}).get("max_reject_tax_pct", 0.0) or 0.0
            ),
            "max_hard_fail_pct": float(
                (assessment.get("thresholds") or {}).get("max_hard_fail_pct", 0.0) or 0.0
            ),
            "min_sample_size": int(
                (assessment.get("thresholds") or {}).get("min_sample_size", 0) or 0
            ),
        },
        "deltas": delta,
