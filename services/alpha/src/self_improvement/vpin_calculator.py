...
class GPUVPINCalculator:
    ...
    def __init__(self, config: VPINConfig):
        ...
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() and config.gpu_acceleration else "cpu"
        )
    ...
    def _preallocate_tensors(self):
        ...
        self.feature_buffer = torch.zeros(
            (self.max_batch_size, self.feature_dim), dtype=torch.float32, device=self.device
        )

        # Pre-allocate volume buckets
        self.volume_buffer = torch.zeros(
            (self.max_batch_size,), dtype=torch.float32, device=self.device
        )
    ...
    async def calculate_vpin_gpu(
        self, trades_df: pd.DataFrame, order_book_df: pd.DataFrame
    ) -> VPINResult:
        """
        ...
        try:
            ...
            vpin_values = await self._calculate_vpin_kernel_gpu(
                volume_buckets, buy_buckets, sell_buckets
            )
    ...
    async def _classify_bulk_volume_gpu(
        self, trades_df: pd.DataFrame, order_book_df: pd.DataFrame
    ) -> dict[str, Any]:
        ...
        return {"bulk_ratio": bulk_ratio, "classifications": classifications}

    def _extract_bulk_features(
        self, trades_df: pd.DataFrame, order_book_df: pd.DataFrame
    ) -> np.ndarray:
        ...
        for _, trade in trades_df.iterrows():
            ...
            price_change = abs(
                trade.get("price", 0) - trade.get("prev_price", trade.get("price", 0))
            )
            ...
            if not order_book_df.empty:
                ...
                bid_ask_spread = trade.get("ask_price", trade.get("price", 1)) - trade.get(
                    "bid_price", trade.get("price", 1)
                )
    ...
    async def _calculate_vpin_kernel_gpu(
        self, volume_buckets: torch.Tensor, buy_buckets: torch.Tensor, sell_buckets: torch.Tensor
    ) -> torch.Tensor:
        ...
        vpin = torch.where(
            total_volume > 0, imbalance / total_volume, torch.zeros_like(total_volume)
        )

        return vpin

    async def _calculate_microstructure_features_gpu(
        self, trades_df: pd.DataFrame, order_book_df: pd.DataFrame, vpin_values: torch.Tensor
    ...
    def _calculate_confidence_score(
        self, vpin_values: torch.Tensor, trades_df: pd.DataFrame
    ) -> float:
    ...
    async def _calculate_vpin_cpu_fallback(
        self, trades_df: pd.DataFrame, order_book_df: pd.DataFrame
    ) -> VPINResult:
...
class RealTimeVPINProcessor:
    ...
    async def add_trade_data(self, symbol: str, trade_data: dict[str, Any]):
        ...
        self.trade_buffer.append(
            {"symbol": symbol, "data": trade_data, "timestamp": datetime.now()}
        )
    ...
    async def _process_buffered_data(self):
        ...
        for symbol, trades in symbol_trades.items():
            try:
                ...
                # Calculate VPIN
                vpin_result = await self.vpin_calculator.calculate_vpin_gpu(
...
async def calculate_vpin_for_symbol(
    symbol: str, trades_df: pd.DataFrame, order_book_df: pd.DataFrame, config: VPINConfig = None
) -> VPINResult:
