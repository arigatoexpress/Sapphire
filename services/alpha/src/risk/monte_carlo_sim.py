...
class MonteCarloSimulator:
    ...
    def __init__(self, config: MonteCarloConfig):
        ...
        logger.info(
            f"[TARGET] Target: ${config.initial_capital:.0f} → ${config.target_capital:.0f}"
        )
        logger.info(f"[DATA] Simulations: {config.num_simulations}")

    def simulate_single_trade(self, capital: float, win_prob: float = None) -> tuple[float, bool]:
        """
        ...
        if is_win:
            ...
            loss_pct = np.random.normal(self.config.avg_loss_pct, self.config.avg_loss_pct * 0.3)
            loss_pct = max(0.05, loss_pct)  # Minimum 0.05%
            pnl = -capital * (self.config.risk_per_trade_pct / 100.0) * loss_pct

        return pnl, is_win

    def simulate_single_day(self, capital: float, trades_per_day: int = None) -> tuple[float, dict]:
        """
    ...
    def simulate_single_path(self, path_id: int = 0, verbose: bool = False) -> dict:
        """
    ...
    def aggregate_results(self) -> dict:
        ...
        days_to_target = [
            r["days_to_target"] for r in self.simulation_results if r["days_to_target"] is not None
        ]
    ...
    def plot_results(self, num_paths_to_plot: int = 100, save_path: str | None = None):
        ...
        try:
            ...
            ax1.set_title(
                f"Capital Growth Paths (showing {num_paths_to_plot} of {len(self.simulation_results)})"
            )
            ax1.set_yscale("log")
            ax1.legend()
            ...
            ax3.axvline(
                x=self.config.max_drawdown_limit * 100, color="red", linestyle="--", label="Limit"
            )
            ...
            days_to_target = [
                ...
                if r["days_to_target"] is not None
            ]
            if days_to_target:
                ax4.hist(days_to_target, bins=30, edgecolor="black", alpha=0.7)
                ax4.axvline(
                    x=np.median(days_to_target), color="red", linestyle="--", label="Median"
                )
...
def run_quick_assessment(
    initial_capital: float = 50.0, target_capital: float = 500000.0, win_prob: float = 0.55
) -> dict:
    """
