...
# Sapphire Brain — cross-silo synthesis + correlation endpoints
# (/api/brain/synthesis, /api/brain/correlate, /api/brain/history).
# Lazy-wired so a brain module failure can't take down the dashboard.
try:
    from brain import register_brain as _register_brain

    _register_brain(app, project=PROJECT, dataset=DATASET, bq_client=bq, query_param_factory=bigquery)
