...
class SwarmManager:
    ...
    def update_agent_state(self, agent_id: str, role: str, sentiment: str, positions: List[str], last_win: float, score: float):
    ...
    def _recalc_global_exposure(self):
        ...
        for agent in self.state.agents.values():
            for symbol in agent.active_positions:
                exposure[symbol] = exposure.get(symbol, 0) + (1 if agent.current_sentiment == "BULLISH" else -1)
    ...
    def get_swarm_context(self, requesting_agent_id: str, symbol: str) -> str:
        ...
        # 1. Peer Sentiment
        bulls = [a.id for a in self.state.agents.values() if a.current_sentiment == "BULLISH" and a.id != requesting_agent_id]
        bears = [a.id for a in self.state.agents.values() if a.current_sentiment == "BEARISH" and a.id != requesting_agent_id]
        ...
        best_agent = max(self.state.agents.values(), key=lambda a: a.performance_score, default=None)
        expert_str = ""
        ...
        exposure_str = "High Long Exposure" if net_exp > 2 else "High Short Exposure" if net_exp < -2 else "Balanced Exposure"
