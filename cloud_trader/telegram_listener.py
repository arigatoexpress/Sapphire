...
class ChatSignalParser:
    ...
    # Sentiment keywords
    BULLISH_KEYWORDS = [
        "buy", "long", "bullish", "moon", "pump", "sending", "breakout",
        "accumulate", "dip buy", "entry", "lfg", "wagmi", "rip", "send it",
        "bullish af", "reversal", "bottomed", "oversold", "🚀", "📈", "🟢", "💚", "🔥",
    ]
    
    BEARISH_KEYWORDS = [
        "sell", "short", "bearish", "dump", "crash", "breakdown", "exit",
        "take profit", "tp hit", "rekt", "ngmi", "overextended", "overbought",
        "top signal", "distribution", "📉", "🔴", "💀", "⚠️", "🩸",
    ]
    ...
    def _extract_symbols(self, text: str) -> Set[str]:
        ...
        for pattern in self.TICKER_PATTERNS:
            ...
            for match in matches:
                ...
                if symbol not in {"THE", "FOR", "AND", "USD", "USDT", "USDC", "A", "I", "IS", "IT", "TO", "OF", "IF"}:
                    symbols.add(symbol)
    ...
    def get_recent_signals(
        ...
        for signal in reversed(self.signal_queue):
            ...
            if symbol:
                ...
                symbol_upper = symbol.upper().replace("-", "").replace("USDT", "").replace("USDC", "")
