...
class MeanReversionStrategy(TradingStrategy):
    ...
    async def evaluate(
        ...
        if cached_bands:
            ...
            await cache.set(
                cache_key, {"upper": upper_band, "lower": lower_band, "sma": sma}, ttl=60
            )
...
class DQNStrategy(TradingStrategy):
    ...
    def __init__(
        ...
        state_size: int = 10,
        action_size: int = 3,
        rl_manager: Optional[RLStrategyManager] = None,
    ):
        super().__init__("DQN")
    ...
    def _construct_state(
        self, market_data: MarketSnapshot, historical_data: pd.DataFrame
    ) -> np.ndarray:
...
class PPOStrategy(TradingStrategy):
    ...
    def __init__(
        self, learning_rate: float = 0.0003, rl_manager: Optional[RLStrategyManager] = None
    ):
        super().__init__("PPO")
...
class StrategySelector:
    ...
    async def select_best_strategy(
        ...
        try:
            logger.debug(
                f"Attempting AI analysis for {symbol} with {len(signals)} existing signals"
            )
            ai_signal = await self._get_ai_analysis_signal(
                symbol, market_data, historical_data, signals
            )
            if ai_signal:
                logger.info(
                    f"✅ AI analysis successful for {symbol}: {ai_signal.direction} (confidence: {ai_signal.confidence:.2f})"
                )
    ...
    async def _get_ai_analysis_signal(
        self,
        symbol: str,
        market_data: MarketSnapshot,
        ...
        try:
            ...
            from .prompt_engineer import PromptBuilder, PromptContext, ResponseValidator
            ...
            logger.debug(
                f"Vertex AI client obtained for {symbol}, existing signals: {len(existing_signals)}"
            )
            ...
            if response and isinstance(response, dict):
                ...
                if validated_response:
                    ...
                    fallback = validator.create_fallback_signal(
                        symbol, "Response validation failed"
                    )
