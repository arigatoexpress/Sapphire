"""Arbitrum protocol registry — typed accessor over ``config/arbitrum_protocols.yaml``.

Exact same shape as :mod:`lib.chains.megaeth.registry`. The split is
deliberate so each chain owns its own yaml — adding a protocol to one
chain doesn't touch the other's source of truth.

Wave 1 populates ``aave_v3`` only. GMX V2, Camelot V3, etc. extend the
same yaml in follow-up PRs.
"""
...
DEFAULT_REGISTRY_PATH = (
    pathlib.Path(__file__).resolve().parents[3] / "config" / "arbitrum_protocols.yaml"
)
...
class ProtocolRegistry:
    """In-memory typed view of ``config/arbitrum_protocols.yaml``."""
