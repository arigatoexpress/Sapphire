...
class CrossChainArbBacktest:
    ...
    def simulate_from_signal(self, signal: AaveApyArbSignal) -> BacktestResult:
        ...
        toward.
        """
        max_spread_bps = float(max(signal.max_supply_spread_bps, signal.max_borrow_spread_bps))
        return self._simulate_with_initial_spread(
            asset=signal.asset, initial_spread_bps=max_spread_bps
        )

    def simulate_with_spread(self, asset: str, initial_spread_bps: float) -> BacktestResult:
    ...
    def _simulate_with_initial_spread(
        ...
        # one round-trip).
        bridge_cost_usd = self.bridge_cost_bps_per_round_trip / 10_000.0 * self.capital_usd
        ...
        for i in range(n_rebalances):
            ...
            gross_period_pnl = spread_bps / 10_000.0 * self.capital_usd * period_years
